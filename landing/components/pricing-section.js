...
function PricingSection() {
    var _a  (0, react_1.useState)(null), openFaq  _a[0], setOpenFaq  _a[1];
    return (<section id"pricing" className"py-20 md:py-32 border-t border-dark-800">
      <div className"container mx-auto px-4">
        ...
        <div className"grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-20">
          {plans.map(function (plan) { return (<div key{plan.id} className{"p-6 rounded-lg border ".concat(plan.highlight
              ...
              <ul className"space-y-3 mb-6">
                {plan.features.map(function (feature) { return (<li key{feature} className"flex items-start gap-2">
              ...
              <button className{"w-full py-3 rounded-lg font-semibold transition-colors ".concat(plan.highlight
        ...
        <div className"max-w-2xl mx-auto">
          ...
          <div className"space-y-4">
            {faqs.map(function (faq, index) { return (<div key{index} className"border border-dark-800 rounded-lg overflow-hidden">
                <button onClick{function () { return setOpenFaq(openFaq  index ? null : index); }} className"w-full px-5 py-4 text-left flex items-center justify-between hover:bg-dark-900 transition-colors">
                  <span className"font-medium text-white">{faq.q}</span>
                  <svg className{"w-5 h-5 text-dark-400 transition-transform ".concat(openFaq  index ? "rotate-180" : "")} fill"none" stroke"currentColor" viewBox"0 0 24 24">
                    <path strokeLinecap"round" strokeLinejoin"round" strokeWidth{2} d"M19 9l-7 7-7-7"/>
                  </svg>
                </button>
                {openFaq  index && (<div className"px-5 pb-4">
