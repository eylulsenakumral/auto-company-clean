...
function Get-RunningAnchorProcess {
    ...
    $pidText ***REMOVED*** (Get-Content $pidFile -ErrorAction SilentlyContinue | Select-Object -First 1).Trim()
    if (-not $pidText) {
        return $null
    }
    ...
    $proc ***REMOVED*** Get-Process -Id $pidValue -ErrorAction SilentlyContinue
    if (-not $proc) {
        return $null
    }
