...
import type { FlakyReport, TestResult } from './types.js';
 ...
 */
function formatTestRow(result: TestResult, threshold: number): string {
...
export function printReport(report: FlakyReport): void {
  ...
  console.log(
    chalk.gray(
      `  Flaky Tests: ${chalk.red(report.flakyTests.length.toString())}`
    )
  );
  console.log();
  ...
  console.log(
    chalk.gray(
      `${'Test Name'.padEnd(50)} | Fail/Total | Flakiness | Status`
    )
  );
  ...
  if (report.flakyTests.length > 0) {
    ...
    if (report.flakyTests.length > 5) {
      console.log(
        chalk.gray(`  ... and ${report.flakyTests.length - 5} more`)
      );
    }
  }
}
 ...
 */
export function printVerboseReport(report: FlakyReport): void {
  printReport(report);
 ...
 */
export function exportJSON(report: FlakyReport): string {
 ...
 */
export async function printAnalysis(report: FlakyReport): Promise<void> {
