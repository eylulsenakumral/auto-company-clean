...
const options ***REMOVED*** {
  ...
  maxSize: 5000,
  sizeCalculation: (value, key) ***REMOVED***> {
    return 1
  },

  // for use when you need to clean up something when objects
  // are evicted from the cache
  dispose: (value, key, reason) ***REMOVED***> {
  ...
  onInsert: (value, key, reason) ***REMOVED***> {
  ...
  fetchMethod: async (key, staleValue, { options, signal, context }) ***REMOVED***> {},
}

const cache ***REMOVED*** new LRUCache(options)
...
const cache ***REMOVED*** {
  ...
  set: (k, v, ttl) ***REMOVED***> {
    ...
    cache.timers.set(
      k,
      setTimeout(() ***REMOVED***> cache.delete(k), ttl),
    )
    cache.data.set(k, v)
  },
  get: k ***REMOVED***> cache.data.get(k),
  has: k ***REMOVED***> cache.data.has(k),
  delete: k ***REMOVED***> {
    if (cache.timers.has(k)) {
      clearTimeout(cache.timers.get(k))
    }
...
```js
import { LRUCache } from 'lru-cache'
const undefinedValue ***REMOVED*** Symbol('undefined')
const cache ***REMOVED*** new LRUCache(...)
const mySet ***REMOVED*** (key, value) ***REMOVED***>
  cache.set(key, value ***REMOVED******REMOVED******REMOVED*** undefined ? undefinedValue : value)
const myGet ***REMOVED*** (key, value) ***REMOVED***> {
  const v ***REMOVED*** cache.get(key)
  return v ***REMOVED******REMOVED******REMOVED*** undefinedValue ? undefined : v
}
```
...
subscribe('lru-cache:metrics', (message, name) ***REMOVED***> {
...
tracingChannel('lru-cache').subscribe({
  start: status ***REMOVED***> {
    // a traced operation is starting
  },
  asyncStart: status ***REMOVED***> {
    // an async traced operation is starting
  },
  asyncEnd: status ***REMOVED***> {
...
const set ***REMOVED*** (k, v) ***REMOVED***> (myCache[k] ***REMOVED*** v)
const get ***REMOVED*** k ***REMOVED***> myCache[k]
...
// mocking timers, e.g. jest.useFakeTimers()
const { LRUCache } ***REMOVED*** await import('lru-cache')
```
...
let myClockTime ***REMOVED*** 0

const cache ***REMOVED*** new LRUCache<string>({
  max: 10,
  ttl: 1000,
  perf: {
    now: () ***REMOVED***> myClockTime,
  },
})
