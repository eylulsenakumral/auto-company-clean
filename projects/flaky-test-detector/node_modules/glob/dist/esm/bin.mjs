...
try {
    ...
    const { cmd, shell, all, default: def, version: showVersion, help, absolute, cwd, dot, 'dot-relative': dotRelative, follow, ignore, 'match-base': matchBase, 'max-depth': maxDepth, mark, nobrace, nocase, nodir, noext, noglobstar, platform, realpath, root, stat, debug, posix, 'cmd-arg': cmdArg, } ***REMOVED*** values;
    if (showVersion) {
        console.log(version);
        process.exit(0);
    }
    ...
    //const { shell, help } ***REMOVED*** values
    if (positionals.length ***REMOVED******REMOVED******REMOVED*** 0 && !def)
        throw 'No patterns provided';
    if (positionals.length ***REMOVED******REMOVED******REMOVED*** 0 && def)
        positionals.push(def);
    const patterns ***REMOVED*** all ? positionals : positionals.filter(p ***REMOVED***> !existsSync(p));
    const matches ***REMOVED*** all ? [] : positionals.filter(p ***REMOVED***> existsSync(p)).map(p ***REMOVED***> join(p));
    const stream ***REMOVED*** globStream(patterns, {
        absolute,
        cwd,
        dot,
        dotRelative,
    ...
    if (!cmd) {
        matches.forEach(m ***REMOVED***> console.log(m));
        stream.on('data', f ***REMOVED***> console.log(f));
    }
    else {
        cmdArg.push(...matches);
        stream.on('data', f ***REMOVED***> cmdArg.push(f));
        ...
        const { SHELL ***REMOVED*** 'unknown' } ***REMOVED*** process.env;
        const shellBase ***REMOVED*** basename(SHELL);
        const knownShells ***REMOVED*** ['sh', 'ksh', 'zsh', 'bash', 'fish'];
        if ((shell || /[ "']/.test(cmd)) &&
            knownShells.includes(shellBase)) {
            const cmdWithArgs ***REMOVED*** `${cmd} "\$${shellBase ***REMOVED******REMOVED******REMOVED*** 'fish' ? 'argv' : '@'}"`;
            if (shellBase !***REMOVED******REMOVED*** 'fish') {
                cmdArg.unshift(SHELL);
            }
            cmdArg.unshift('-c', cmdWithArgs);
            stream.on('end', () ***REMOVED***> foregroundChild(SHELL, cmdArg));
        }
        else {
            if (shell) {
            ...
            stream.on('end', () ***REMOVED***> foregroundChild(cmd, cmdArg, { shell }));
        }
    }
}
catch (e) {
...
//# sourceMappingURL***REMOVED***bin.mjs.map