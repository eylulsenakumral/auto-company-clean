...
const jsfiles ***REMOVED*** await glob('**/*.js', { ignore: 'node_modules/**' })

// pass in a signal to cancel the glob walk
const stopAfter100ms ***REMOVED*** await glob('**/*.css', {
  signal: AbortSignal.timeout(100),
})

// multiple patterns supported as well
const images ***REMOVED*** await glob(['css/*.{png,jpeg}', 'public/*.{png,jpeg}'])

// but of course you can do that with the glob pattern also
// the sync function is the same, just returns a string[] instead
// of Promise<string[]>
const imagesAlt ***REMOVED*** globSync('{css,public}/*.{png,jpeg}')

// you can also stream them, this is a Minipass stream
const filesStream ***REMOVED*** globStream(['**/*.dat', 'logs/**/*.log'])

// construct a Glob object if you wanna do it that way, which
// allows for much faster walks if you have to look in the same
// folder multiple times.
const g ***REMOVED*** new Glob('**/foo', {})
...
const g2 ***REMOVED*** new Glob('**/bar', g)
...
const g3 ***REMOVED*** new Glob('**/baz/**', { withFileTypes: true })
g3.stream().on('data', path ***REMOVED***> {
  console.log(
    'got a path object',
    path.fullpath(),
    path.isDirectory(),
    path.readdirSync().map(e ***REMOVED***> e.name),
  )
})
...
// slower, though.
// For example:
const results ***REMOVED*** await glob('**', { stat: true, withFileTypes: true })

const timeSortedFiles ***REMOVED*** results
  .sort((a, b) ***REMOVED***> a.mtimeMs - b.mtimeMs)
  .map(path ***REMOVED***> path.fullpath())

const groupReadableFiles ***REMOVED*** results
  .filter(path ***REMOVED***> path.mode & 0o040)
  .map(path ***REMOVED***> path.fullpath())

// custom ignores can be done like this, for example by saying
// you'll ignore all markdown files, and all folders named 'docs'
const customIgnoreResults ***REMOVED*** await glob('**', {
  ignore: {
    ignored: p ***REMOVED***> /\.md$/.test(p.name),
    childrenIgnored: p ***REMOVED***> p.isNamed('docs'),
  },
})

// another fun use case, only return files with the same name as
// their parent folder, plus either `.ts` or `.js`
const folderNamedModules ***REMOVED*** await glob('**/*.{ts,js}', {
  ignore: {
    ignored: p ***REMOVED***> {
      const pp ***REMOVED*** p.parent
...
const newFiles ***REMOVED*** await glob('**', {
...
paths found.
  ...
  -D<n> --max-depth***REMOVED***<n>  Maximum depth to traverse from the current working
                         directory

  -C<cwd> --cwd***REMOVED***<cwd>    Current working directory to execute/match in
  -r<root> --root***REMOVED***<root> A string path resolved against the 'cwd', which is used
                         ...
                         path, you can use '--root***REMOVED***' to set it to the empty
                         string. However, be aware that on Windows systems, a
                         pattern like 'x:/*' or '//host/share/*' will *always*
                         start in the 'x:/' or '//host/share/' directory,
                         regardless of the --root setting.

  --platform***REMOVED***<platform>  Defaults to the value of 'process.platform' if
                         available, or 'linux' if not. Setting --platform***REMOVED***win32
                         on non-Windows systems may cause strange behavior!

  -i<ignore> --ignore***REMOVED***<ignore>
...
## `glob(pattern: string | string[], options?: GlobOptions) ***REMOVED***> Promise<string[] | Path[]>`
...
## `globSync(pattern: string | string[], options?: GlobOptions) ***REMOVED***> string[] | Path[]`

Synchronous form of `glob()`.

Alias: `glob.sync()`

## `globIterate(pattern: string | string[], options?: GlobOptions) ***REMOVED***> AsyncGenerator<string>`

Return an async iterator for walking glob pattern matches.

Alias: `glob.iterate()`

## `globIterateSync(pattern: string | string[], options?: GlobOptions) ***REMOVED***> Generator<string>`

Return a sync iterator for walking glob pattern matches.

Alias: `glob.iterate.sync()`, `glob.sync.iterate()`

## `globStream(pattern: string | string[], options?: GlobOptions) ***REMOVED***> Minipass<string | Path>`
...
## `globStreamSync(pattern: string | string[], options?: GlobOptions) ***REMOVED***> Minipass<string | Path>`
...
## `hasMagic(pattern: string | string[], options?: GlobOptions) ***REMOVED***> boolean`
...
## `escape(pattern: string, options?: GlobOptions) ***REMOVED***> string`
...
## `unescape(pattern: string, options?: GlobOptions) ***REMOVED***> string`
...
### `const g ***REMOVED*** new Glob(pattern: string | string[], options: GlobOptions)`
...
- `includeChildMatches` boolean, default `true`. Do not match any
  ...
  ```js
  const results ***REMOVED*** await glob(
    [
...
`[***REMOVED***c***REMOVED***]` and `[.symbol.]` style class patterns are not.
...
but `[***REMOVED***c***REMOVED***]` (locale-specific character collation weight), and
...
<small id***REMOVED***"fn-webscale">[1]: In the cases where this module
