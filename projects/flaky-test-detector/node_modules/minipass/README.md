...
[It's very
fast](https://docs.google.com/spreadsheets/d/1K_HR5oh3r80b8WVMWCPPjfuWXUgfkmhlX7FGI6JJ8tY/edit?usp***REMOVED***sharing)
...
export class NDJSONStream extends Minipass<string, any, Events> {
  ...
  write(data, encoding, cb) {
    try {
      const json ***REMOVED*** JSON.stringify(data)
      return super.write(json + '\n', encoding, cb)
    } catch (er) {
      if (!er instanceof Error) {
        er ***REMOVED*** Object.assign(new Error('json stringify failed'), {
          cause: er,
        })
      }
...
const s ***REMOVED*** new NDJSONStream()
s.on('jsonError', e ***REMOVED***> {
...
const stream ***REMOVED*** new Minipass()
stream.on('data', () ***REMOVED***> console.log('data event'))
...
options, or by setting `stream.async ***REMOVED*** true` later on.
...
const asyncStream ***REMOVED*** new Minipass({ async: true })
asyncStream.on('data', () ***REMOVED***> console.log('data event'))
console.log('before write')
...
```js
import { Minipass } from 'minipass'
const stream ***REMOVED*** new Minipass({ encoding: 'utf8' })
stream.on('data', chunk ***REMOVED***> console.log(chunk))
stream.async ***REMOVED*** true
...
console.log('before writes')
stream.write('hello')
stream.async ***REMOVED*** false // <-- no-op, stream already async
stream.write('world')
console.log('after writes')
// actual output:
// before writes
...
const p1 ***REMOVED*** new PassThrough({ highWaterMark: 1024 })
const p2 ***REMOVED*** new PassThrough({ highWaterMark: 1024 })
const p3 ***REMOVED*** new PassThrough({ highWaterMark: 1024 })
const p4 ***REMOVED*** new PassThrough({ highWaterMark: 1024 })

p1.pipe(p2).pipe(p3).pipe(p4)
p4.on('data', () ***REMOVED***> console.log('made it through'))
...
```js
const m1 ***REMOVED*** new Minipass()
const m2 ***REMOVED*** new Minipass()
const m3 ***REMOVED*** new Minipass()
const m4 ***REMOVED*** new Minipass()

m1.pipe(m2).pipe(m3).pipe(m4)
m4.on('data', () ***REMOVED***> console.log('made it through'))
...
```js
const tee ***REMOVED*** new Minipass()
t.pipe(dest1)
t.pipe(dest2)
...
const src ***REMOVED*** new Minipass()
src.write('foo')
...
`stream.async ***REMOVED*** true` afterwards. Note that this does add some
...
```js
import { Minipass } from 'minipass'
const mp ***REMOVED*** new Minipass(options) // options is optional
mp.write('foo')
...
mp.collect().then(all ***REMOVED***> {
  ...
  // const data ***REMOVED*** await stream.collect()
})
```
...
```js
mp.concat().then(onebigchunk ***REMOVED***> {
...
```js
const mp ***REMOVED*** new Minipass({ objectMode: true })
mp.write('a')
mp.write('b')
...
```js
const mp ***REMOVED*** new Minipass({ encoding: 'utf8' })

// some source of some data
let i ***REMOVED*** 5
const inter ***REMOVED*** setInterval(() ***REMOVED***> {
...
consume().then(res ***REMOVED***> console.log(res))
// logs `foo\n` 5 times, and then `ok`
```
...
class SlowEnd extends Minipass {
  emit(ev, ...args) {
    if (ev ***REMOVED******REMOVED******REMOVED*** 'end') {
...
class NDJSONEncode extends Minipass {
  write(obj, cb) {
    ...
    if (typeof obj ***REMOVED******REMOVED******REMOVED*** 'function') {
      cb ***REMOVED*** obj
      obj ***REMOVED*** undefined
    }
    if (obj !***REMOVED******REMOVED*** undefined) {
      this.write(obj)
    }
    return super.end(cb)
  }
...
class NDJSONDecode extends Minipass {
  constructor(options) {
    ...
    super({ objectMode: true })
    this._jsonBuffer ***REMOVED*** ''
  }
  write(chunk, encoding, cb) {
    if (
      typeof chunk ***REMOVED******REMOVED******REMOVED*** 'string' &&
      typeof encoding ***REMOVED******REMOVED******REMOVED*** 'string' &&
      encoding !***REMOVED******REMOVED*** 'utf8'
    ) {
      chunk ***REMOVED*** Buffer.from(chunk, encoding).toString()
    } else if (Buffer.isBuffer(chunk)) {
      chunk ***REMOVED*** chunk.toString()
    }
    if (typeof encoding ***REMOVED******REMOVED******REMOVED*** 'function') {
      cb ***REMOVED*** encoding
    }
    const jsonData ***REMOVED*** (this._jsonBuffer + chunk).split('\n')
    this._jsonBuffer ***REMOVED*** jsonData.pop()
    for (let i ***REMOVED*** 0; i < jsonData.length; i++) {
      try {
