...
[![Build Status](https://github.com/tj/commander.js/workflows/build/badge.svg)](https://github.com/tj/commander.js/actions?query***REMOVED***workflow%3A%22build%22)
[![NPM Version](http://img.shields.io/npm/v/commander.svg?style***REMOVED***flat)](https://www.npmjs.org/package/commander)
[![NPM Downloads](https://img.shields.io/npm/dm/commander.svg?style***REMOVED***flat)](https://npmcharts.com/compare/commander?minimal***REMOVED***true)
[![Install Size](https://packagephobia.now.sh/badge?p***REMOVED***commander)](https://packagephobia.now.sh/result?p***REMOVED***commander)
...
const options ***REMOVED*** program.opts();
const limit ***REMOVED*** options.first ? 1 : undefined;
...
$ node string-util.js split --separator***REMOVED***/ a/b/c
[ 'a', 'b', 'c' ]
```
...
An option and its option-argument can be separated by a space, or combined into the same argument. The option-argument can follow the short option directly or follow an `***REMOVED***` for a long option.

```sh
serve -p 80
serve -p80
serve --port 80
serve --port***REMOVED***80
```
...
- small pizza size
- vegetarian
$ pizza-options --pizza-type***REMOVED***cheese
pizza details:
- cheese
```
...
const options ***REMOVED*** program.opts();
const sauceStr ***REMOVED*** options.sauce ? 'sauce' : 'no sauce';
const cheeseStr ***REMOVED*** (options.cheese ***REMOVED******REMOVED******REMOVED*** false) ? 'no cheese' : `${options.cheese} cheese`;
...
$ pizza-options --cheese***REMOVED***blue
...
const options ***REMOVED*** program.opts();
if (options.cheese ***REMOVED******REMOVED******REMOVED*** undefined) console.log('no cheese');
else if (options.cheese ***REMOVED******REMOVED******REMOVED*** true) console.log('add cheese');
...
So `id` behaves as a boolean option for `--id -5`, but you can use a combined form if needed like `--id***REMOVED***-5`.
...
Remaining arguments:  []
$ collect --letter***REMOVED***A -n80 operand
...
$ PORT***REMOVED***80 extra --donate --free-drink
...
function myParseInt(value, dummyPrevious) {
  ...
  const parsedValue ***REMOVED*** parseInt(value, 10);
...
const options ***REMOVED*** program.opts();
if (options.float !***REMOVED******REMOVED*** undefined) console.log(`float: ${options.float}`);
if (options.integer !***REMOVED******REMOVED*** undefined) console.log(`integer: ${options.integer}`);
if (options.verbose > 0) console.log(`verbosity: ${options.verbose}`);
if (options.collect.length > 0) console.log(options.collect);
if (options.list !***REMOVED******REMOVED*** undefined) console.log(options.list);
```
...
program
  ...
  .action((source, destination) ***REMOVED***> {
...
program
  ...
  .action((username, password) ***REMOVED***> {
...
program
  ...
  .action((first, second) ***REMOVED***> {
    console.log(`${first} + ${second} ***REMOVED*** ${first + second}`);
  })
;
```
...
program
  ...
  .action((name, options, command) ***REMOVED***> {
    if (options.debug) {
      console.error('Called %s with options %o', command.name(), options);
    }
    const title ***REMOVED*** options.title ? `${options.title} ` : '';
...
program
  ...
  .hook('preAction', (thisCommand, actionCommand) ***REMOVED***> {
...
```js
program.name('pizza');
const pm ***REMOVED*** new Command('pm');
```
...
program.configureHelp({
  ...
  subcommandTerm: (cmd) ***REMOVED***> cmd.name() // Just show the name, instead of short usage.
});
```
...
program.on('option:verbose', function () {
  process.env.VERBOSE ***REMOVED*** this.opts().verbose;
});
```
...
With pass through options, the `--port***REMOVED***80` is a program option in the first line and passed through as a command-argument in the second line:

```sh
program --port***REMOVED***80 arg
program arg --port***REMOVED***80
```
...
program
  ...
  .option('-d, --debug')
  .action((commandAndOptions) ***REMOVED***> {
...
const program ***REMOVED*** createCommand();
```
...
program
  .configureOutput({
    ...
    writeOut: (str) ***REMOVED***> process.stdout.write(`[OUT] ${str}`),
    writeErr: (str) ***REMOVED***> process.stdout.write(`[ERR] ${str}`),
    // Highlight errors in color.
    outputError: (str, write) ***REMOVED***> write(errorColor(str))
  });
```
...
The maintainers of Commander and thousands of other packages are working with Tidelift to deliver commercial support and maintenance for the open source dependencies you use to build your applications. Save time, reduce risk, and improve code health, while paying the maintainers of the exact dependencies you use. [Learn more.](https://tidelift.com/subscription/pkg/npm-commander?utm_source***REMOVED***npm-commander&utm_medium***REMOVED***referral&utm_campaign***REMOVED***enterprise&utm_term***REMOVED***repo)
