...
- [Posix character
  ...
  matching is not supported, so `[[***REMOVED***e***REMOVED***]]` will _not_ match `'é'`
...
```javascript
var Minimatch ***REMOVED*** require('minimatch').Minimatch
var mm ***REMOVED*** new Minimatch(pattern, options)
```
...
```javascript
var isJS ***REMOVED*** minimatch(file, '*.js', { matchBase: true })
```
...
```javascript
var javascripts ***REMOVED*** fileList.filter(
...
```javascript
var javascripts ***REMOVED*** minimatch.match(fileList, '*.js', { matchBase: true })
```
...
minimatch('/x/y/z', '/a/**/z', { partial: true }) // false, because x !***REMOVED******REMOVED*** a
```
