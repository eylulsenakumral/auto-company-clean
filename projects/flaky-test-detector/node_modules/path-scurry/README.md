...
const pw ***REMOVED*** new PathScurry(process.cwd())
for await (const entry of pw) {
  if (entry.isFile() && entry.name ***REMOVED******REMOVED******REMOVED*** '.DS_Store') {
    unlink(entry.fullpath())
  }
}

// here it is as a manual recursive method
const walk ***REMOVED*** async (entry: Path) ***REMOVED***> {
  const promises: Promise<any> ***REMOVED*** []
  ...
  for (const child of await pw.readdir(entry)) {
    ...
    if (child.name ***REMOVED******REMOVED******REMOVED*** '.DS_Store' && child.isFile()) {
...
walk(pw.cwd).then(() ***REMOVED***> {
  console.log('all .DS_Store files removed')
})

const pw2 ***REMOVED*** new PathScurry('/a/b/c') // pw2.cwd is the Path for /a/b/c
const relativeDir ***REMOVED*** pw2.cwd.resolve('../x') // Path entry for '/a/b/x'
const relative2 ***REMOVED*** pw2.cwd.resolve('/a/b/d/../x') // same path, same entry
...
- `filter`: Function `(entry: Path) ***REMOVED***> boolean`. If provided,
...
- `walkFilter`: Function `(entry: Path) ***REMOVED***> boolean`. If provided,
...
#### `async pw.readdir(dir ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: true })`
...
#### `pw.readdirSync(dir ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: true })`

Synchronous `pw.readdir()`

#### `async pw.readlink(link ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: false })`
...
#### `pw.readlinkSync(link ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: false })`

Synchronous `pw.readlink()`

#### `async pw.lstat(entry ***REMOVED*** pw.cwd)`
...
#### `pw.lstatSync(entry ***REMOVED*** pw.cwd)`

Synchronous `pw.lstat()`

#### `pw.realpath(entry ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: false })`
...
#### `pw.realpathSync(entry ***REMOVED*** pw.cwd, opts ***REMOVED*** { withFileTypes: false })`
