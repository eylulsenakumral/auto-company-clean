...
type StyleObject ***REMOVED*** Record<string, string | number | undefined>;
...
function defaultTemplate(params: OGParams, watermark: boolean): VNode {
  const { title, description, domain, author, tag, theme ***REMOVED*** 'dark' } ***REMOVED*** params;
  const isDark ***REMOVED*** theme ***REMOVED******REMOVED******REMOVED*** 'dark';

  const bg ***REMOVED*** isDark ? '#0A0A0A' : '#FAFAFA';
  const primary ***REMOVED*** isDark ? '#F5F5F5' : '#0A0A0A';
  const secondary ***REMOVED*** isDark ? '#737373' : '#737373';
  const accent ***REMOVED*** '#F59E0B';
  const surface ***REMOVED*** isDark ? '#1A1A1A' : '#E8E8E8';

  const fontSize ***REMOVED*** title.length > 60 ? '42px' : title.length > 40 ? '52px' : '62px';
...
function blogTemplate(params: OGParams, watermark: boolean): VNode {
  ...
  const bg ***REMOVED*** isDark ? '#0D0D0D' : '#FFFFFF';
  const primary ***REMOVED*** isDark ? '#FAFAFA' : '#111111';
  const secondary ***REMOVED*** isDark ? '#6B7280' : '#6B7280';
  const accent ***REMOVED*** '#F59E0B';
  const surface ***REMOVED*** isDark ? '#1F1F1F' : '#F3F4F6';

  const fontSize ***REMOVED*** title.length > 55 ? '44px' : title.length > 35 ? '54px' : '64px';
...
function articleTemplate(params: OGParams, watermark: boolean): VNode {
  ...
  const bg ***REMOVED*** isDark ? '#111111' : '#F8F8F8';
  const primary ***REMOVED*** isDark ? '#FFFFFF' : '#111111';
  const secondary ***REMOVED*** isDark ? '#9CA3AF' : '#4B5563';
  const accent ***REMOVED*** '#F59E0B';
  const _surface ***REMOVED*** isDark ? '#222222' : '#E5E7EB';
  void _surface;
  const divider ***REMOVED*** isDark ? '#2A2A2A' : '#D1D5DB';

  const fontSize ***REMOVED*** title.length > 60 ? '40px' : title.length > 40 ? '50px' : '60px';
