...
```
TAM ***REMOVED*** Total Market Category Size
SAM ***REMOVED*** TAM × Geographic % × Segment %
SOM ***REMOVED*** SAM × Realistic Capture Rate (2-5%)
```
...
```
TAM ***REMOVED*** Σ (Segment Size × Annual Revenue per Customer)
SAM ***REMOVED*** TAM × (Segments You Can Serve / Total Segments)
SOM ***REMOVED*** SAM × Realistic Penetration Rate (Year 3-5)
```
...
```
Value per Customer ***REMOVED*** Problem Cost × % Solved by Solution
Price per Customer ***REMOVED*** Value × Willingness to Pay % (10-30%)
TAM ***REMOVED*** Total Potential Customers × Price per Customer
SAM ***REMOVED*** TAM × % Meeting Buy Criteria
SOM ***REMOVED*** SAM × Realistic Adoption Rate
```
