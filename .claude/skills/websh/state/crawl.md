...
def cd(url):
    ...
    if env.EAGER_CRAWL:
        ...
        Task(
            description***REMOVED***f"websh: eager crawl {slug}",
            prompt***REMOVED***EAGER_CRAWL_PROMPT.format(
                url***REMOVED***full_url,
                slug***REMOVED***slug,
                parsed_path***REMOVED***f".websh/cache/{slug}.parsed.md",
                depth***REMOVED***env.CRAWL_DEPTH,
                same_domain***REMOVED***env.CRAWL_SAME_DOMAIN,
                max_per_page***REMOVED***env.CRAWL_MAX_PER_PAGE,
                max_concurrent***REMOVED***env.CRAWL_MAX_CONCURRENT,
                delay_ms***REMOVED***env.CRAWL_DELAY_MS,
            ),
            subagent_type***REMOVED***"general-purpose",
            model***REMOVED***"haiku",
            run_in_background***REMOVED***True
...
Current crawl:
  ...
  [✓] .../item?id***REMOVED***41234567
  [✓] .../item?id***REMOVED***41234568
  [→] .../item?id***REMOVED***41234569 (fetching...)
  ...
...
def should_crawl(url, domain):
    robots ***REMOVED*** get_robots(domain)  # cached
