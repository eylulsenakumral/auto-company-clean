...
```python
# BAD: Wide-open CORS
app.add_middleware(CORSMiddleware, allow_origins***REMOVED***["*"])

# GOOD: Explicit allowed origins
app.add_middleware(
    CORSMiddleware,
    allow_origins***REMOVED***["https://app.example.com"],
    allow_methods***REMOVED***["GET", "POST", "PUT", "DELETE"],
    allow_headers***REMOVED***["Authorization", "Content-Type"],
)

# BAD: Debug mode in production
app ***REMOVED*** FastAPI(debug***REMOVED***True)

# GOOD: Debug only in development
app ***REMOVED*** FastAPI(debug***REMOVED***settings.DEBUG)  # DEBUG***REMOVED***False in production
```
...
token ***REMOVED*** jwt.encode({"sub": user_id}, SECRET_KEY, algorithm***REMOVED***"HS256")

# GOOD: JWT with expiration
token ***REMOVED*** jwt.encode(
    {"sub": user_id, "exp": datetime.utcnow() + timedelta(minutes***REMOVED***30)},
    SECRET_KEY,
    algorithm***REMOVED***"HS256",
)
```
...
url ***REMOVED*** request.query_params["url"]
response ***REMOVED*** httpx.get(url)  # SSRF: can access internal services

# GOOD: Validate URL against allowlist
ALLOWED_HOSTS ***REMOVED*** {"api.example.com", "cdn.example.com"}
parsed ***REMOVED*** urlparse(url)
if parsed.hostname not in ALLOWED_HOSTS:
    raise HTTPException(400, "URL not allowed")
response ***REMOVED*** httpx.get(url)
```
...
| `subprocess.run(cmd, shell***REMOVED***True)` | Command injection | Pass args as list, `shell***REMOVED***False` |
...
| `jwt.decode(token, options***REMOVED***{"verify_signature": False})` | Auth bypass | Always verify signature |
