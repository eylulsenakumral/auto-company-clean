...
class CitationVerifier:
    ...
    def verify_url(self, url: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            req ***REMOVED*** request.Request(url, method***REMOVED***'HEAD')
            req.add_header('User-Agent', 'Mozilla/5.0 (Research Citation Verifier)')

            with request.urlopen(req, timeout***REMOVED***10) as response:
                if response.status ***REMOVED******REMOVED*** 200:
    ...
    def detect_hallucination_patterns(self, entry: Dict) -> List[str]:
        ...
        generic_words ***REMOVED*** ['overview', 'introduction', 'guide', 'handbook', 'manual']
        ...
        if entry.get('year'):
            year ***REMOVED*** int(entry['year'])
            # Very recent without DOI or URL is suspicious
            if year >***REMOVED*** 2024 and not entry.get('doi') and not entry.get('url'):
    ...
    def check_title_similarity(self, title1: str, title2: str) -> float:
        ...
        def normalize(s):
            s ***REMOVED*** s.lower()
            s ***REMOVED*** re.sub(r'[^\w\s]', ' ', s)
            return set(s.split())

        words1 ***REMOVED*** normalize(title1)
        words2 ***REMOVED*** normalize(title2)

        if not words1 or not words2:
            return 0.0

        overlap ***REMOVED*** len(words1 & words2)
        total ***REMOVED*** len(words1 | words2)
    ...
    def verify_entry(self, entry: Dict) -> Dict:
        ...
        hallucination_issues ***REMOVED*** self.detect_hallucination_patterns(entry)
        ...
        # STEP 2: Has DOI?
        if entry['doi']:
            print(f"  [{entry['num']}] Checking DOI {entry['doi']}...", end***REMOVED***' ')
            success, metadata ***REMOVED*** self.verify_doi(entry['doi'])

            if success:
                result['metadata'] ***REMOVED*** metadata
                result['status'] ***REMOVED*** 'verified'
                print("")

                # Check title similarity if we have both
                if entry['title'] and metadata.get('title'):
                    similarity ***REMOVED*** self.check_title_similarity(
                        entry['title'],
                        metadata['title']
                    )
                ...
                if entry['year'] and metadata.get('year'):
                    if int(entry['year']) !***REMOVED*** int(metadata['year']):
                        result['issues'].append(
