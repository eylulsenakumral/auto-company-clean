...
rclone copy large-video.mp4 r2:my-bucket/ \
  --s3-upload-cutoff***REMOVED***100M \
  --s3-chunk-size***REMOVED***100M
```
...
wrangler r2 bucket notification create my-bucket \
  ...
  --event-type***REMOVED***object-create
```
...
wrangler r2 bucket sippy enable my-bucket \
  --provider***REMOVED***aws \
  --bucket***REMOVED***source-bucket \
  --region***REMOVED***us-east-1 \
  --access-key-id***REMOVED***$AWS_KEY \
  --secret-access-key***REMOVED***$AWS_SECRET
```
...
- Larger multipart chunks ***REMOVED*** fewer Class A operations
