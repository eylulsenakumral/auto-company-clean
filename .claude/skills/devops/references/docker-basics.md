...
ENV NODE_ENV***REMOVED***production          # Environment variables
...
docker run -e NODE_ENV***REMOVED***production myapp:1.0
...
WORKDIR /app
COPY --from***REMOVED***build /usr/local/lib/python3.11/site-packages /usr/local/lib/python3.11/site-packages
COPY . .
...
RUN go mod download
COPY . .
RUN CGO_ENABLED***REMOVED***0 go build -o main .

FROM scratch
COPY --from***REMOVED***build /app/main /main
CMD ["/main"]
```
...
# Set ownership
COPY --chown***REMOVED***nodejs:nodejs . .
