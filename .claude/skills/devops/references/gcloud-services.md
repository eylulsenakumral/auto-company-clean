...
gcloud functions deploy my-function \
  --runtime***REMOVED***python39 \
  --trigger-http \
  --allow-unauthenticated \
  --entry-point***REMOVED***main
...
# Load data
bq load --source_format***REMOVED***CSV my_dataset.my_table \
  gs://my-bucket/data.csv \
  schema.json

# Query
bq query --use_legacy_sql***REMOVED***false \
...
```bash
# Submit build
gcloud builds submit --tag***REMOVED***gcr.io/PROJECT_ID/my-image
...
gcloud artifacts repositories create my-repo \
  --repository-format***REMOVED***docker \
  --location***REMOVED***us-central1
...
gcloud compute networks create my-network \
  --subnet-mode***REMOVED***auto

# Create firewall rule
gcloud compute firewall-rules create allow-http \
  --network***REMOVED***my-network \
  --allow***REMOVED***tcp:80
...
gcloud projects add-iam-policy-binding PROJECT_ID \
  --member***REMOVED***"user:user@example.com" \
  --role***REMOVED***"roles/viewer"
...
```bash
# View logs
gcloud logging read "resource.type***REMOVED***gce_instance" \
  --limit***REMOVED***10 \
  --format***REMOVED***json
...
gcloud logging sinks create my-sink \
  ...
  --log-filter***REMOVED***"resource.type***REMOVED***gce_instance"
