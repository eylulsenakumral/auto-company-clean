...
# YAML
gcloud compute instances list --format***REMOVED***yaml

# CSV
gcloud compute instances list --format***REMOVED***"csv(name,zone,status)"

# Value (single field)
gcloud config get-value project --format***REMOVED***"value()"

# Custom table
gcloud compute instances list \
  --format***REMOVED***"table(name,zone,machineType,status)"
```
...
gcloud compute instances list --filter***REMOVED***"zone:us-central1-a"
gcloud compute instances list --filter***REMOVED***"status***REMOVED***RUNNING"
gcloud compute instances list --filter***REMOVED***"name~^web-.*"

# Multiple conditions
gcloud compute instances list \
  --filter***REMOVED***"zone:us-central1 AND status***REMOVED***RUNNING"

# Negation
gcloud compute instances list --filter***REMOVED***"NOT status***REMOVED***TERMINATED"
```
      ...
      - name: Deploy
        run: |
          gcloud run deploy my-service \
            --image***REMOVED***gcr.io/${{ secrets.GCP_PROJECT_ID }}/my-image \
...
deploy:
  ...
  script:
    ...
    - gcloud auth activate-service-account --key-file***REMOVED***key.json
...
- Limit output: `--limit***REMOVED***10`
- Project only needed fields: `--format***REMOVED***"value(name)"`
...
gcloud projects get-iam-policy PROJECT_ID \
  --flatten***REMOVED***"bindings[].members" \
  --filter***REMOVED***"bindings.members:user@example.com"
