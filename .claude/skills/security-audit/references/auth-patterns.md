...
```typescript
const MAX_ATTEMPTS ***REMOVED*** 5;
const LOCKOUT_DURATION ***REMOVED*** 15 * 60 * 1000; // 15 minutes

async function recordLoginAttempt(email: string, success: boolean): Promise<void> {
  const key ***REMOVED*** `login_attempts:${email}`;
  ...
  const attempts ***REMOVED*** await redis.incr(key);
  
  if (attempts ***REMOVED******REMOVED******REMOVED*** 1) {
    await redis.expire(key, LOCKOUT_DURATION / 1000);
  }
}

async function isAccountLocked(email: string): Promise<boolean> {
  const attempts ***REMOVED*** await redis.get(`login_attempts:${email}`);
  return parseInt(attempts || '0') >***REMOVED*** MAX_ATTEMPTS;
}
...
async function requestPasswordReset(email: string): Promise<void> {
  const user ***REMOVED*** await User.findByEmail(email);
  
  // Don't reveal if email exists
  if (!user) {
    // Still "process" to prevent timing attacks
    await new Promise(r ***REMOVED***> setTimeout(r, 100 + Math.random() * 100));
    return;
  }
  
  // Generate secure token
  const token ***REMOVED*** crypto.randomBytes(32).toString('hex');
  const tokenHash ***REMOVED*** crypto.createHash('sha256').update(token).digest('hex');
...
async function resetPassword(token: string, newPassword: string): Promise<void> {
  ...
  const reset ***REMOVED*** await PasswordReset.findOne({
    tokenHash,
    used: false,
  ...
  reset.used ***REMOVED*** true;
  await reset.save();
  
  // Update password
  const hash ***REMOVED*** await hashPassword(newPassword);
...
async function initiateOAuth(req: Request, res: Response) {
  const state ***REMOVED*** crypto.randomBytes(32).toString('hex');
  
  // Store state in session
  req.session.oauthState ***REMOVED*** state;
  req.session.oauthReturnTo ***REMOVED*** req.query.returnTo || '/';
  
  const authUrl ***REMOVED*** new URL('https://provider.com/oauth/authorize');
...
async function handleOAuthCallback(req: Request, res: Response) {
  const { code, state } ***REMOVED*** req.query;
  
  // Verify state
  if (state !***REMOVED******REMOVED*** req.session.oauthState) {
  ...
  const tokens ***REMOVED*** await exchangeCode(code as string);
  
  // Verify ID token
  const claims ***REMOVED*** await verifyIdToken(tokens.id_token);
  
  // Create or update user
  const user ***REMOVED*** await upsertOAuthUser(claims);
